...
Verified against live HTML (2026-08-30):

Economic /calendar/economic?dateFrom=<Monday>
  route-init-data.data.entries = LIST of events for that week bucket.
  Keys: calendarId, ticker, event, category, date, actual, previous,
  forecast, teforecast, importance (1-3).

Earnings /calendar/earnings?dateFrom=<Monday>
  route-init-data.data.entries = DICT {items, page, pageSize,
  totalItemsCount, totalPages}. Only one page of ~5-50 names.

Earnings /calendar/earnings/season-preview
  route-init-data.data.entries = LIST (~200 reports, ~3-4 weeks).
  Keys: date, ticker, company, marketCap, earningsDate, epsEstimate.

Futures /futures.ashx
  `var tiles = {...};` — 49 roots. Brent is QA (not BZ). Bitcoin is BTC
  (not BT). Keep-list is headline order; leftovers still go on the tape.
"""
...
def _econ_pool(data: dict, html: str) -> list[dict]:
    pool: list[dict] = []
...
def _earnings_pool(data: dict, html: str) -> list[dict]:
    pool: list[dict] = []
    entries = data.get("entries") if isinstance(data, dict) else None
    if isinstance(entries, list):
