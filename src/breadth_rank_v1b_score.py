"""Score breadth_rank_v1b one session at a time.
...
from src.breadth_rank_v1b_append import file_sha256, manifest_line
from src.breadth_rank_v1b_bars import load_bars
from src.breadth_rank_v1b_grid import (
    ATOMS,
    BASKETS,
    HOLD,
    LUCK_DENOMINATOR,
...
from src.breadth_rank_v1b_protocol import (
    BAR_BLOB_SHA,
    BAR_PATH,
    BAR_SHA256,
    FIT_CUTOFF,
...
def _rank_rows(rows: list[dict]) -> list[dict]:
    """Long-only pick key: Futubull compound, then rule id. Ex-best is not the key."""
...
def freeze_payload(series: dict) -> dict:
    ...
    for index, rid, _basket, _atoms, side, _top in rules:
        ...
        (longs if side == "long" else shorts).append(row)
    _rank_rows(longs)
    _rank_rows(shorts)
    rank1 = longs[0]["id"] if longs else None
    label = longs[0]["label"] if longs else "untestable"
    closed = longs[0]["closed_trades"] if longs else 0
    return {
        "fit_cutoff": FIT_CUTOFF,
        "nothing_fitted_after": FIT_CUTOFF,
        "objective": "long futubull compound through 2026-09-11",
        "rank1": rank1,
...
def _write_report(view: BarView, fees: dict) -> None:
    ...
    if (
        fresh["rank1"] != frozen.get("rank1")
        or fresh["rows"] != frozen.get("rows")
        or fresh["shorts"] != frozen.get("shorts")
    ):
    ...
    rows_out = [pack(row) for row in frozen["rows"]]
    shorts_out = [pack(row) for row in frozen.get("shorts") or []]
    summary = {
        "label": STUDY_LABEL,
...
def _rule_table(row: dict) -> str:
    header = (
        "| window | compound | up | down | flat | days entered | entries | closed | win rate | ex-best | top-ticker share |\n"
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n"
    )
    lines = [header]
    for key, label in (("before", "before 2026-09-14"), ("after", "after 2026-09-14")):
        stats = row[key]
        lines.append(
            "| {label} | {compound} | {up} | {down} | {flat} | {days} | {entries} | {closed} | {win} | {ex} | {share} |\n".format(
                label=label,
...
def _section(title: str, rows: list[dict]) -> list[str]:
    lines = [f"## {title}", ""]
    if not rows:
        lines.append("No ranked rule.")
        lines.append("")
        return lines
    for row in rows:
        lines.append(f"### {row['rank']}. `{row['id']}` ({row['label']})")
        lines.append("")
        lines.append(_rule_table(row))
        lines.append("")
    return lines
...
def _render(summary: dict) -> str:
    ...
    if rank1:
        lines.append("## Rank 1")
        lines.append("")
        lines.append(_rule_table(rank1))
        lines.append("")
        for label, stats, luck in (
            ...
            lines.append(
                ...
                f"up/down/flat {stats['up']}/{stats['down']}/{stats['flat']}, "
                f"days entered {stats['days_entered']}, entries {stats['entries']}, "
        ...
        lines.append("")
    lines.extend(_section("Long rules", rows))
    lines.extend(_section("Short rules", summary.get("shorts") or []))
