...
WORKABLE_ALWAYS = (
    ...
    # Fee-KEEP pins (research cards). Cash-book Win% can fail WORKABLE_BAR
    # while after-fee prove still KEEP — do not strand them off Pages.
    "union_e_fresh_h3",            # FACTOR_LONG_FEE_KEEP #282
    "short_clk_neg_weak_fail_h3",  # catalogue c4_downside #278
)
FEE_KEEP_FEATURED = (
    "union_e_fresh_h3",
...
def prune_payload_workable(payload: dict, bar: dict | None = None) -> dict:
    ...
    if isinstance(payload.get("featured"), list):
        ...
        # Fee-KEEP pins lead the featured strip even when cash Win% failed the bar.
        pinned = [n for n in FEE_KEEP_FEATURED if n in keep]
        rest = [n for n in (feat or sorted(keep)) if n not in pinned]
        out["featured"] = pinned + rest
    ...
    out["workable"] = {
        ...
        "note": (
            ...
            "Fee-KEEP pins (union_e_fresh_h3, short_clk_neg_weak_fail_h3/"
            "c4_downside) always kept. Rest discarded."
        ),
    }
    return out
...
def load_dash_payload() -> dict:
    """Prefer full scoreboard+shards so remine KEEP pins are not stranded.

    Baked HTML is already prune_payload_workable'd; restoring fee-KEEP /
    WORKABLE_ALWAYS pins requires the unpruned scoreboard. Fall back to
    baked HTML only when the scoreboard is missing.
    """
    if OUT_JSON.is_file():
        doc = load_scoreboard()
        if doc and doc.get("stats"):
            return doc
...
def restamp_dash() -> dict:
    ...
    Prefers the full scoreboard+shards (post-remine) over a previously
    pruned baked HTML so fee-KEEP / WORKABLE_ALWAYS pins are not stranded
    on an older workable trim. Falls back to baked HTML when scoreboard
    is missing. Rebuilds the sim pack from the panel when available.
    """
    payload = load_scoreboard() or load_dash_payload()
    if not payload:
