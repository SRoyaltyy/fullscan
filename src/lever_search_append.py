"""Append-only guard for the Group 3 return record.

Each session is written once under research/lever_search/returns/factor_mine_seq/.
...
RETURNS_DIR = Path("research/lever_search/returns/factor_mine_seq")
...
def assert_append_only(
    ...
    base_files and head_files map a per-day filename to its raw bytes.
    """
...
def check_against(rev: str) -> None:
    """Compare the working tree with the return record at git rev."""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Fail if an earlier factor_mine_seq session changed.")
