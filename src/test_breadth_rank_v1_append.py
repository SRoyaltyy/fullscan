"""CI guard: breadth_rank_v1 may only gain sessions after the last recorded one."""
from __future__ import annotations

from pathlib import Path

from src.breadth_rank_v1_append import (
    MANIFEST_NAME,
    RETURNS_DIR,
    AppendOnlyError,
    assert_append_only,
...
def _day(session: str, body: bytes, *, blob: str = BLOB) -> tuple[str, bytes, str]:
    ...
    line = manifest_line({
        ...
        "input_blobs": [{"path": "research/breadth_rank_v1/bars/ohlc.parquet", "blob_sha": blob}],
    })
...
def test_workflow_runs_the_check() -> None:
    text = (ROOT / ".github" / "workflows" / "breadth_rank_v1_append_only.yml").read_text(encoding="utf-8")
    assert "src.breadth_rank_v1_append --check-against" in text
    assert "src.test_breadth_rank_v1_append" in text
    assert "src.breadth_rank_v1_bars --check" in text
    assert str(RETURNS_DIR) == "research/breadth_rank_v1/returns"
