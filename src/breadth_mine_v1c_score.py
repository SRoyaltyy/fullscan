"""Score the fingerprinted breadth_mine_v1c grid.
...
from src.breadth_mine_v1c_append import file_sha256, manifest_line
from src.breadth_mine_v1c_bars import session_jumps, unexplained
...
from src.breadth_mine_v1c_metrics import (
    compound,
    ex_best_compound,
    luck_p,
    median,
...
from src.breadth_mine_v1c_protocol import (
    BAR_BLOB_SHA,
    BAR_PATH,
    BAR_SHA256,
    CREATION_DATE,
...
def walk_all(days: list[dict], fees: dict) -> dict:
    from src.breadth_mine_v1c_grid import rule_id
...
def evaluate(scored: dict, ids: list[str]) -> list[dict]:
    ...
    for ri, rule in enumerate(ids):
        ...
        check = [
            sessions[di] for di in range(len(sessions))
            if sessions[di] in check_cal and sessions[di] in run and not bool(scored["sat"][ri, di])
        ]
        check_set = set(check)
        ...
        n_check = len(check)
        lines = {
            "a": removed is not None and removed >= 0.10,
            "b": med is not None and med > 0.0,
            "c": win is not None and win > WIN_MIN,
            "d": from14 >= 0.0,
            "e": adj < 0.05,
            "f": not rejected,
            "g": entries >= MIN_FIRES,
        ...
        if total_entries == 0:
            label = "untestable"
        elif all(lines.values()) and n_check < 10:
            label = "too few days to judge"
        elif all(lines.values()) and all(day < CREATION_DATE for day in check):
            label = "designed_after"
        elif all(lines.values()):
...
def render_report(rows: list[dict], verdict: str) -> str:
    eligible = [row for row in rows if not row["rejected"] and row["label"] != "untestable"]
    ranked = sorted(
        eligible,
        key=lambda row: (
            -(row["ex_best"] if row["ex_best"] is not None else -1e9),
            -row["full"],
            row["id"],
        ),
    )
    ...
    def cell(row: dict, rank: int) -> str:
        return "| " + " | ".join([
            str(rank),
            f"`{row['id']}`",
            row["side"],
    ...
    header = [
        ...
        "",
        "Ranked on the ex-best-ticker Futubull compound on each rule's check days. "
        "Check days are search days from 2026-08-20 through 2026-09-11. "
        "The book is continuous from 2026-08-13. "
        "A rule whose top-1 ticker is above 50% of check-day P&L is rejected and is not in this table.",
        "",
        ...
        "",
        f"Rules scored: {len(rows)}. Rejected for top-1 share above 50%: {sum(1 for row in rows if row['rejected'])}. "
        ...
        "## Top 20",
        "",
        "| rank | rule | side | check days | full compound | ex-best | best ticker | median trade | win rate | top-1 share | top-3 share | trades/day | fires | random4 beaten | iwm | from 09-14 | under $3 | 15bp compound | raw p | luck p | label |",
        "| ---: | --- | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
...
def attach_baselines(rows: list[dict], scored: dict, days: list[dict], fees: dict, bars: Bars) -> None:
    """IRONCLAD 19. Same check days, Futubull fees."""
    sessions = scored["sessions"]
    ...
    for row in rows:
        if row["label"] == "untestable":
            continue
        idx = [
            di for di, session in enumerate(sessions)
            if session in check_cal and session in run and not bool(scored["sat"][row["index"], di])
        ]
        if not idx:
            row["random4_beaten"] = None
            row["iwm"] = None
            continue
        rule_c = _compound_idx([float(scored["ret_f"][row["index"], di]) for di in range(len(sessions))], idx)
        beaten = 0
        total = 0.0
        for draw_i in range(RANDOM4_DRAWS):
            base = _compound_idx(matrix[draw_i], idx)
            total += base
            if rule_c > base:
                beaten += 1
        row["random4_beaten"] = beaten / RANDOM4_DRAWS
        row["random4_mean"] = total / RANDOM4_DRAWS
        row["iwm"] = _compound_idx(iwm, idx)
...
def main() -> None:
    assert_prereg()
    _verify_bar_pin()
    from src.breadth_mine_v1c_bars import assert_split_consistent
