"""CI guard: breadth_mine_v1b may only gain sessions after the last recorded one."""
from __future__ import annotations

from pathlib import Path

from src.breadth_mine_v1b_append import (
    MANIFEST_NAME,
    RETURNS_DIR,
    AppendOnlyError,
    assert_append_only,
...
def test_workflow_runs_the_check() -> None:
    text = (ROOT / ".github" / "workflows" / "breadth_mine_v1b_append_only.yml").read_text(encoding="utf-8")
    assert "src.breadth_mine_v1b_append --check-against" in text
    assert "src.test_breadth_mine_v1b_append" in text
    assert "src.test_breadth_mine_v1b" in text
    assert str(RETURNS_DIR) == "research/breadth_mine_v1b/returns"
