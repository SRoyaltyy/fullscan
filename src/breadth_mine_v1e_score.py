"""Score the fingerprinted breadth_mine_v1e grid.
...
from src.breadth_mine_v1e_append import file_sha256, manifest_line
from src.breadth_mine_v1e_bars import session_jumps, unexplained
from src.breadth_mine_v1e_grid import (
    ATOM_ROLES,
    ATOMS,
    CANDIDATE_CAP,
    CUT_LOSS,
...
from src.breadth_mine_v1e_metrics import (
    compound,
    ex_best_compound,
    luck_p,
    median,
...
from src.breadth_mine_v1e_protocol import (
    BAR_BLOB_SHA,
    BAR_PATH,
    BAR_SHA256,
    CREATION_DATE,
...
def advance_book(book: Book, *, side: str, exit_name: str, hold: int, min_hold: int,
    ...
    for ticker in list(book.pos):
        ...
        missing = px is None or px <= 0
        if missing:
            px = float(lot["prev"])
        if px is None or px <= 0:
            continue
        if not missing and not want_exit(held, hold, min_hold, exit_name, side, px, lot, ticker in universe):
            continue
        shares = lot["shares"]
...
def walk_all(days: list[dict], fees: dict) -> dict:
    from src.breadth_mine_v1e_grid import rule_id
...
def apply_objective(row: dict, check_before_creation: bool) -> None:
    """Pre-09-14 guards. Luck and the forward window do not enter."""
    if row["label"] == "untestable":
        row["failed"] = []
        row["primary_hit"] = False
        row["selection_pass"] = False
        return
    ...
    row["selection_pass"] = not row["failed"] and row["primary_hit"] and row["n_check"] >= 10
    if row["n_check"] < 10:
        row["label"] = "too few days to judge"
    elif row["selection_pass"] and check_before_creation:
        row["label"] = "eligible"
    elif row["selection_pass"]:
        row["label"] = "eligible"
    else:
        row["label"] = "not selected"


def select_pick(rows: list[dict]) -> dict | None:
    """Rank 1 by pre-window Futubull compound. Luck is not a key."""
    pool = [row for row in rows if row.get("selection_pass")]
    pool.sort(key=lambda row: (-row["full"], row["id"]))
...
def _iwm_returns(bars: Bars, sessions: list[str], fees: dict) -> list[float]:
    ...
    if lot is not None and out and sessions and sessions[-1] == "2026-09-25":
...
def _check_idx(sessions: list[str]) -> list[int]:
    return [i for i, session in enumerate(sessions) if session in set(CHECK_CALENDAR)]


def _oos_idx(sessions: list[str]) -> list[int]:
    return [i for i, session in enumerate(sessions) if OOS_START <= session <= "2026-09-25"]


def _rule_on_idx(scored: dict, ri: int, idx: list[int], ids: list[str],
                 random_mean: float | None, iwm_c: float | None) -> dict:
    sessions = scored["sessions"]
    rets = [float(scored["ret_f"][ri, i]) for i in range(len(sessions))]
    check = [sessions[i] for i in idx]
    pnl_by_day: dict[str, dict[str, float]] = {}
    totals: dict[str, float] = {}
    trades: list[float] = []
    fires = 0
    for i in idx:
        session = sessions[i]
        for ticker, value in scored["day_pnl"][i].get(ri, {}).items():
            pnl_by_day.setdefault(session, {})[ticker] = float(value)
            totals[ticker] = totals.get(ticker, 0.0) + float(value)
        trades.extend(scored["day_trades"][i].get(ri, []))
        fires += int(scored["n_entries"][ri, i])
    best = None
    if totals:
        best = min(totals, key=lambda ticker: (-totals[ticker], scored["first"][ri].get(ticker, "9999-99-99"), ticker))
    total = sum(totals.values())
    top1 = None
    if total > 0 and best is not None:
        top1 = max(totals.values()) / total
    wins = sum(1 for value in trades if value > 0)
    return {
        "id": ids[ri],
        "index": ri,
        "label": "untestable" if int(scored["n_entries"][ri].sum()) == 0 else "",
        "full": compound([rets[i] for i in idx]) if idx else 0.0,
        "ex_best": ex_best_compound(sessions, check, rets, pnl_by_day, best),
        "best": best or "",
        "fires": fires,
        "win": (wins / len(trades)) if trades else None,
        "top1": top1,
        ...
        "luck_p": luck_p([rets[i] for i in idx], LUCK_DENOMINATOR) if idx else 1.0,
        "raw_p": raw_luck_p([rets[i] for i in idx]) if idx else 1.0,
        "random4_mean": random_mean,
        "iwm": iwm_c,
        "from_0914": 0.0,
        "side": _side(ids[ri]),
    }
...
def walk_forward(scored: dict, days: list[dict], fees: dict, bars: Bars, ids: list[str]) -> list[dict]:
    """Re-pick each pre-window day from earlier window sessions only."""
    sessions = scored["sessions"]
    idx = _check_idx(sessions)
    iwm = _iwm_returns(bars, sessions, fees)
    matrix = _random4_matrix(days, fees)
    path = []
    equity = 1.0
    for k, di in enumerate(idx):
        prefix = idx[:k]
        if not prefix:
            path.append({"session": sessions[di], "rule": "cash", "ret": 0.0, "path": equity - 1.0, "label": "walk_forward"})
            continue
        draw_c = [_compound_idx(matrix[draw_i], prefix) for draw_i in range(RANDOM4_DRAWS)]
        random_mean = sum(draw_c) / len(draw_c)
        iwm_c = _compound_idx(iwm, prefix)
        chosen = None
        for ri in range(N):
            row = _rule_on_idx(scored, ri, prefix, ids, random_mean, iwm_c)
            apply_objective(row, True)
            if chosen is None or (row.get("selection_pass") and (
                not chosen.get("selection_pass") or row["full"] > chosen["full"] or (
                    row["full"] == chosen["full"] and row["id"] < chosen["id"]
                )
            )):
                if row.get("selection_pass"):
                    chosen = row
        if chosen is None:
            ret = 0.0
            rule = "cash"
        else:
            ret = float(scored["ret_f"][chosen["index"], di])
            rule = chosen["id"]
        equity *= 1.0 + ret
        path.append({
            "session": sessions[di],
        ...
        print(f"walk-forward {sessions[di]} {rule}", flush=True)
    return path


def forward_block(scored: dict, ri: int) -> dict:
    sessions = scored["sessions"]
    idx = _oos_idx(sessions)
    rets = [float(scored["ret_f"][ri, i]) for i in range(len(sessions))]
    check = [sessions[i] for i in idx]
    pnl_by_day: dict[str, dict[str, float]] = {}
    totals: dict[str, float] = {}
    ...
    for i in idx:
        ...
        for ticker, value in scored["day_pnl"][i].get(ri, {}).items():
            ...
            totals[ticker] = totals.get(ticker, 0.0) + float(value)
        trades.extend(scored["day_trades"][i].get(ri, []))
        if int(scored["n_fills"][ri, i]) > 0:
            fill_days += 1
    best = None
    if totals:
        best = min(totals, key=lambda ticker: (-totals[ticker], scored["first"][ri].get(ticker, "9999-99-99"), ticker))
    return {
        "compound": compound([rets[i] for i in idx]) if idx else 0.0,
        "day_count": len(idx),
        "ex_best": ex_best_compound(sessions, check, rets, pnl_by_day, best),
        "fill_days": fill_days,
        "trades": len(trades),
...
def hindsight_best(scored: dict, ids: list[str]) -> dict | None:
    sessions = scored["sessions"]
    idx = _oos_idx(sessions)
    best = None
    for ri, rule in enumerate(ids):
        value = compound([float(scored["ret_f"][ri, i]) for i in idx]) if idx else 0.0
        if best is None or value > best["compound"] or (value == best["compound"] and rule < best["id"]):
            best = {"id": rule, "compound": value, "label": "hindsight"}
    return best


def study_verdict(pick: dict | None, forward: dict | None) -> str:
    if pick is None:
        return "nothing selected"
    compound_fwd = 0.0 if forward is None else float(forward["compound"])
    if compound_fwd > 0.0:
        return "win"
    return "no win"
...
def render_verdict(pick: dict | None, forward: dict | None, path: list[dict],
    ...
    lines = [
        "# Breadth mine v1e",
        "",
        "Nothing used to form the pick was fitted after 2026-09-13. "
        "Atoms, thresholds, the grid, and the rank are the fingerprinted prereg. "
        "The rank uses 2026-08-20 through 2026-09-11 only. "
        "Luck p is shown and does not decide the verdict.",
        "",
        f"Verdict: `{verdict}`",
        "",
        "Every session is before the creation date 2026-09-26, so this is `designed_after` as a live record.",
        "",
        "## The pick",
        "",
    ]
    if pick is None:
        ...
        lines += [
            f"- rule: `{pick['id']}`",
            f"- side: {pick.get('side', '')}",
            f"- pre-09-14 Futubull compound: {pct(pick['full'])}",
            f"- pre-09-14 ex-best: {pct(pick.get('ex_best'))}",
            f"- fires: {pick.get('fires')}",
            f"- win rate: {pct(pick.get('win'))}",
            f"- top-1 share: {pct(pick.get('top1'))}",
            f"- RANDOM4 mean: {pct(pick.get('random4_mean'))}",
            f"- IWM: {pct(pick.get('iwm'))}",
            f"- raw p: {pick.get('raw_p')}",
            f"- luck p: {pick.get('luck_p')}",
            "",
            "Forward result, 2026-09-14 through 2026-09-25. This did not choose the rule.",
            "",
        ]
        if forward is None:
            lines.append("Forward result missing.")
        else:
            lines += [
                f"- Futubull compound: {pct(forward['compound'])}",
                f"- ex-best: {pct(forward['ex_best'])}",
                f"- trade count: {forward['trades']}",
                f"- day count: {forward['day_count']}",
            ]
    lines += ["", "## Walk-forward, labelled `walk_forward`", "",
              "| session | rule | day return | path compound |",
              "| --- | --- | ---: | ---: |"]
    for row in path:
        lines.append(f"| {row['session']} | `{row['rule']}` | {pct(row['ret'])} | {pct(row['path'])} |")
    if not path:
        lines.append("|  |  |  |  |")
    lines += ["", "## Hindsight-best, labelled `hindsight`", ""]
...
def main() -> None:
    assert_prereg()
    _verify_bar_pin()
    from src.breadth_mine_v1e_bars import assert_split_consistent
    ...
    print("loading formation bars through 2026-09-13", flush=True)
    search_bars = Bars(max_date=FORMATION_END)
    for dates, *_rest in search_bars.rows.values():
        if dates and dates[-1] > FORMATION_END:
            raise SystemExit("formation tape includes a bar after 2026-09-13")
    days = _build_phase(search_bars, grouped, search, cache)
    scored_pre = walk_all(days, fees)
    ids = _ids()
    print("rank on the pre-09-14 window", flush=True)
    rows_pre = evaluate(scored_pre, ids)
    attach_baselines(rows_pre, scored_pre, days, fees, search_bars)
    pick = select_pick(rows_pre)
    path = walk_forward(scored_pre, days, fees, search_bars, ids)
    pick_path = ROOT / "research" / "breadth_mine_v1e" / "PICK.json"
    payload = {
        "label": "pick",
        "rule": None if pick is None else pick["id"],
        "pre_compound": None if pick is None else pick["full"],
    }
    pick_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"froze pick {payload['rule']}", flush=True)
    print("loading forward bars after the pick is on disk", flush=True)
    oos_bars = Bars()
    ...
    again = select_pick(rows)
    again_id = None if again is None else again["id"]
    if again_id != payload["rule"]:
        raise SystemExit(f"forward pass changed the pick {payload['rule']} -> {again_id}")
    forward = None if pick is None else forward_block(scored, pick["index"])
