"""Append-only guard for factor_mine_seq_clean_tape.

Each session is written once under research/lever_search_clean/returns/.
...
RETURNS_DIR = Path("research/lever_search_clean/returns")
...
def manifest_line(obj: dict) -> str:
    """One canonical manifest line. The recorded line is this text, not a re-dump."""
...
def _require_pin(obj: dict) -> None:
    ...
    for item in blobs:
        ...
        if not isinstance(blob, str) or BLOB_SHA_RE.fullmatch(blob) is None:
            raise AppendOnlyError("input snapshot blob sha missing")
        if not isinstance(commit, str) or BLOB_SHA_RE.fullmatch(commit) is None:
            raise AppendOnlyError("input snapshot commit missing")
...
def assert_append_only(
    ...
    """Fail unless head is base plus sessions strictly after the last recorded one."""
...
def check_against(rev: str) -> None:
    """Compare the working tree with the clean-tape return record at git rev."""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Fail if an earlier factor_mine_seq_clean_tape session changed."
    )
