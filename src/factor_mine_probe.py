...
def _card(row: dict, flat: dict | None, news: dict) -> dict:
    ...
    return {
        ...
        "macd_hist": _round(row.get("macd_hist"), 4),
        "rsi_os": bool(row.get("rsi_os")),
        "rsi_ob": bool(row.get("rsi_ob")),
        "macd_up": bool(row.get("macd_up")),
        "macd_cross_up": bool(row.get("macd_cross_up")),
        "flow_in": bool(row.get("flow_in")),
