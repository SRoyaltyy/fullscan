"""Score the fingerprinted breadth_mine_v1b grid.
...
from src.breadth_mine_v1b_append import file_sha256, manifest_line
from src.breadth_mine_v1b_grid import (
    ATOM_ROLES,
    ATOMS,
    CUT_LOSS,
    EXITS,
...
from src.breadth_mine_v1b_metrics import (
    compound,
    ex_best_compound,
    luck_p,
    median,
...
def evaluate(scored: dict, ids: list[str]) -> list[dict]:
    ...
    for ri, rule in enumerate(ids):
        ...
        lines = {
            ...
            "c": win is not None and win > 0.50,
            "d": from14 >= 0.0,
            "e": adj < 0.05,
            "f": not rejected,
        }
        if all(lines.values()) and n_check < 10:
...
def render_report(rows: list[dict], verdict: str) -> str:
    eligible = [row for row in rows if not row["rejected"]]
    ranked = sorted(
        eligible,
        key=lambda row: (
...
def bar_audit_section() -> str:
    ...
    lines = [
        "## Bars",
        "",
        "Fresh Yahoo snapshot, `auto_adjust=false` (split-adjusted, dividends not applied). "
        f"Parquet sha256 `{audit['parquet_sha256']}`. Splits sha256 `{audit['splits_sha256']}`.",
        "",
        f"Prior fills whose price matches the pinned session open and whose fresh open differs: "
        f"#357 {audit['fills']['357']['changed']} of {audit['fills']['357']['n']}, "
        f"#358 {audit['fills']['358']['changed']} of {audit['fills']['358']['n']}, "
        f"together {audit['fills']['changed']}.",
        "",
        f"Flagged consecutive open/close ratios: {len(audit['flags'])}. "
        f"Unexplained: {audit['unexplained']}.",
        "",
    ...
    for row in audit["flags"]:
        ...
        lines.append(
            f"| {row['ticker']} | {row['date']} | {row['leg']} | {row['ratio']} | {split} | {str(row['explained']).lower()} |"
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    bars = Bars()
    print(f"tickers {len(bars.rows)}", flush=True)
    cache: dict[str, bytes] = {}
    days = []
    for session in SESSIONS:
        print(f"build {session}", flush=True)
        day = build_day(bars, grouped[session], session, cache)
        day["session"] = session
        days.append(day)
