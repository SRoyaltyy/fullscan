...
(function(){
  ...
  function names(rows){
    ...
    return rows.map(function(x){
      ...
      var t = (x && (x.ticker || x.symbol)) || "";
      if(!t) return "";
      var px = (x && x.px != null) ? x.px : null;
      var op = (x && x.open_px != null) ? x.open_px : null;
      var bit = t;
      if(px != null) bit += " " + px;
    ...
    if(keys.length){
      ...
      keys.forEach(function(name){
        ...
        if(res && res.tag){
          ...
          var so = (res.short_only || []).map(function(r){ return r.ticker; }).filter(Boolean);
          var dp = (res.dip_scoop || []).map(function(r){ return r.ticker; }).filter(Boolean);
          if(so.length) extra += " short-only " + so.join(" · ");
          if(dp.length) extra += " dip-scoop " + dp.join(" · ");
          extra += "</span>";
        }
        var preds = s.predict || [];
        if(preds.length){
