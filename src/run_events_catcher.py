...
from .run_events import EVENTS_MAX_TOKENS, _repair_json, extract_json, _windows
...
def main() -> None:
    ...
    if replacing:
        ...
        mission = (
            ...
            "Then sweep the secondary categories.\n\n"
            "OUTPUT ORDER (non-negotiable):\n"
            "1. Emit the fenced ```json block FIRST (use key missed_events for "
            "the full list — the merger treats them as the day's events).\n"
            "2. Then any prose.\n"
            "If you run out of space, truncate prose, NEVER the JSON."
        )
    else:
        ...
        mission = (
            ...
            "Then sweep the secondary categories.\n\n"
            "OUTPUT ORDER (non-negotiable):\n"
            "1. Emit the fenced ```json block FIRST (missed_events key).\n"
            "2. Then any prose.\n"
            "If you run out of space, truncate prose, NEVER the JSON."
        )
    ...
    n0 = len(missed)
    print(f"[catcher] primary OpenClaw parse: "
    ...
    if not missed:
        ...
        if ds:
            text = ds
        n1 = len(missed)
        print(f"[catcher] DeepSeek rescan parse: "
