...
class ApplyCase(unittest.TestCase):
    ...
    def test_crash_rebound_admit_clips_and_admits(self):
        ...
        df, meta = apply_to_frame(_frame(), "2026-09-15", shadow=True)
    ...
    def test_micro_admit_bypasses_size_gate_flag(self):
        self.write_registry({"crash_rebound_admit": {
            "enabled": True, "lessons": ["l1.md"], "params": {}}})
        df, _ = apply_to_frame(_frame(), "2026-09-15", shadow=True)
...
class ProspectiveGateCase(ApplyCase):
    def test_unvalidated_override_does_not_change_live_ranker(self):
        self.write_registry({"crash_rebound_admit": {
            "enabled": True, "lessons": ["l1.md"], "params": {}}})
        df, meta = apply_to_frame(_frame(), "2026-09-15")
        self.assertEqual(self.row(df, "CAPR")["s_ab_lx"], -0.76)
        self.assertFalse(bool(self.row(df, "CAPR")["lesson_admit"]))
        self.assertEqual(meta["overrides"]["crash_rebound_admit"]["mode"], "shadow")


class VerdictCase(unittest.TestCase):
    def test_thresholds(self):
        self.assertEqual(lesson_exec._verdict(6, 0.01, False), "positive trigger association")
        self.assertEqual(lesson_exec._verdict(5, 0.0, False), "negative trigger association")
        self.assertEqual(lesson_exec._verdict(5, -0.02, False), "negative trigger association")
