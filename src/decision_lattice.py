...
NEGATIVE_RE = re.compile(
    ...
    r"investigat(?:e|es|ion)|warning|plunges?|selloff|recall)\b",
    re.I,
)
...
def _judge_context(date: str) -> tuple[dict[str, str], dict[str, str], dict]:
    ...
    sectors = {
        ...
        for sec, value in (data.get("sector_tilts") or {}).items()
    }
...
def _parent_eval(row: pd.Series, context: dict) -> dict:
    ...
    essay_value = _num(row.get("s_sector"))
...
def _company_eval(row: pd.Series, context: dict) -> dict:
    ...
    if catalyst.get("usable"):
        ...
        strength = 0.72 if materiality == "high" else 0.48
        sources.append("finviz_digest")
        reasons.append(
            f"direct {materiality} digest: {digest.get('text')}"
        )
        if judge_tone == tone:
...
def _setup_eval(row: pd.Series) -> dict:
    join = _num(row.get("s_join"))
    ab = _num(row.get("s_ab_intrinsic"), _num(row.get("s_ab")))
    if ab <= -0.05 or join <= -0.25:
        tone = "bad"
    elif ab >= 0.10 and join >= 0.0:
        tone = "good"
    else:
        tone = "neutral"
    strength = float(np.clip((ab + join + 1.0) / 3.0, 0.0, 1.0))
    return {
        "tone": tone,
        "strength": round(strength, 3),
        "reasons": [
            f"intrinsic AB={ab:+.2f}",
            f"join/structure={join:+.2f}",
        ],
    }
...
def summarize(df: pd.DataFrame, context: dict, top_n: int = 15) -> dict:
    ...
    bull = df.sort_values("bull_rank", ascending=False).head(top_n)
