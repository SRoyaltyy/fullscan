...
def _load_parsed(date_str: str) -> dict:
    ...
    if p.exists():
        ...
        if isinstance(data, dict) and (
                data.get("usable_top") or data.get("all_items")
                or int(data.get("raw_count") or 0) > 0):
            return data
    return _fallback_parse(date_str)


def _fallback_parse(date_str: str) -> dict:
    """When news_parse timed out, judge still has events + digest + actions."""
    items: list[dict] = []
    ev = Path("01_daily/events") / f"{date_str}_events.json"
    if ev.is_file():
        try:
            payload = json.loads(ev.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            payload = {}
        for e in (payload.get("events") or [])[:40]:
            if not isinstance(e, dict):
                continue
            title = e.get("title") or e.get("event") or e.get("name") or ""
            if title:
                items.append({"title": str(title), "source": "events",
                              "usable": True, "polarity": e.get("dir") or ""})
    digest = Path(NEWS_DIR) / f"{date_str}_finviz_digest.json"
    if digest.is_file():
        try:
            payload = json.loads(digest.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            payload = {}
        for row in (payload.get("index_digests") or [])[:8]:
            if isinstance(row, dict) and row.get("digest"):
                items.append({"title": str(row.get("digest")),
                              "source": "finviz_digest", "usable": True,
                              "polarity": ""})
        for row in (payload.get("top_signal") or [])[:20]:
            if isinstance(row, dict) and (row.get("digest") or row.get("news_title")):
                items.append({
                    "title": str(row.get("digest") or row.get("news_title")),
                    "source": "finviz_digest", "usable": True, "polarity": "",
                })
    actions = Path(NEWS_DIR) / f"{date_str}_actions.json"
    if actions.is_file():
        try:
            payload = json.loads(actions.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            payload = {}
        for evn in (payload.get("reasoned_events") or [])[:12]:
            if not isinstance(evn, dict):
                continue
            name = evn.get("event") or ""
            evd = evn.get("evidence") or []
            title = name
            if evd and isinstance(evd[0], dict):
                title = evd[0].get("title") or name
            if title:
                items.append({"title": str(title), "source": "news_actions",
                              "usable": True,
                              "polarity": str((evn.get("framework") or {}).get("polarity") or "")})
    if not items:
        return {}
    print(f"[news_judge] no parsed.json — fallback {len(items)} items "
          f"from events/digest/actions")
    return {
        "raw_count": len(items),
...
def _build_user_msg(date_str: str, report: dict) -> str:
    ...
    return (
        ...
        "high-signal ticker digests as elevated, pre-validated themes.\n"
    )
...
def main() -> None:
    ...
    if not report:
        print(f"[news_judge] no parsed report and no events/digest fallback for {date_str}")
        return
    ...
    if backup:
        ...
        print("[news_judge] restored previous quality-ok copy")
        return
    output_qc.reject(path)
