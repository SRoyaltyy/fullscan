"""Executable lesson proposals with prospective validation.

Live application requires a code/version-matched promotion record. Historical
trigger-set excess returns describe association only; they cannot approve an
override. Explicit shadow=True evaluates without activating it. Paired portfolio
observations are collected separately by learning_replay.
"""
...
def apply_to_frame(df, date: str, *, shadow: bool = False, override_ids=None, context=None):
    ...
    meta: dict = {"date": date, "overrides": {}, "prose_only": [], "mode": "shadow" if shadow else "validated_only"}
    ...
    for base in ("s_join", "s_news", "s_ab", "s_peer"):
        col = f"{base}_lx"
        if True:  # recompute; never retain stale overrides on a reused frame
    ...
    state = load_state()
    earn_days = (_earn_series(df, date) if context is None else
                 df["Ticker"].astype(str).str.upper().map(context.get("earnings", {})))
    ...
    for oid, spec in registry.items():
        ...
        if not spec.get("enabled") or entry["suspended"] or not active_files:
            continue
        if override_ids is not None and oid not in override_ids:
            continue
        approved = override_approved(oid, spec, date)
        entry["mode"] = "active" if approved else "shadow"
        before = df.copy(deep=True)
        p = spec.get("params") or {}
        try:
            if oid == "news_attach" and digest is None:
                digest = _digest_map(date) if context is None else context.get("digest", {})
            ...
            df = before
        if not shadow and not approved:
            df = before
...
def override_trial(oid, spec):
    from .research_validation import digest
    # Bind evidence to code, parameters and source lesson contents.
    content = {f: (LESSON_DIR / f).read_text() if (LESSON_DIR / f).exists() else None
               for f in spec.get("lessons", [])}
    candidate = {"override": oid, "params": spec.get("params", {}),
                 "code_hash": digest(Path(__file__).read_text()), "lessons": content}
    return {"override": oid, "enabled": False}, candidate


def override_approved(oid, spec, date):
    from .research_validation import digest
    from .learning_trials import TRIALS, trial_key
    base, candidate = override_trial(oid, spec)
    key = trial_key(base, candidate)
    path = TRIALS / "lessons" / oid / "promotions" / (key + ".json")
    try:
        record = json.loads(path.read_text())
        return record["candidate_hash"] == key and record["promoted_at"][:10] < date
...
def _verdict(n_realized: int, mean_excess: float, suspended: bool) -> str:
    ...
    if n_realized < MIN_REALIZED_DATES:
        return "collecting"
    return "positive trigger association" if mean_excess > 0 else "negative trigger association"
...
def grade(date: str, lookback_days: int = GRADE_LOOKBACK_DAYS) -> dict:
    ...
    try:
        ...
        if panel is not None:
            ...
            panel = panel.loc[panel.index <= date].copy()
            if opens is not None:
                panel.attrs["opens"] = opens.reindex(panel.index)
            for d in dates:
                try:
    ...
    for oid, spec in registry.items():
        ...
        from .learning_trials import govern, TRIALS
        base, candidate = override_trial(oid, spec)
        _, trial = govern(base, candidate, date, root=TRIALS / "lessons" / oid)
        entry["prospective_validation"] = trial
        entry["grading_kind"] = "historical trigger association, not causal lesson uplift"
