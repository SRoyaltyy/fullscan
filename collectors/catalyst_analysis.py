...
# ── Gemini call helper (with retry & rate‑limit backoff) ──
def _gemini_api_call(prompt_text, max_output_tokens=4096):
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 429:
                wait = 30 * (attempt + 1)
                print(f"  ⚠️  Gemini rate‑limited, waiting {wait}s…")
                time.sleep(wait)
                continue
...
Verify every catalyst entry below. Use Google Search.

HITs: {json.dumps(hits, indent=2)}
MISSes: {json.dumps(misses, indent=2)}
N/A: {json.dumps(nas, indent=2)}
...
{{
  "corrected_grid": [ {{
    ...
    "evidence_excerpt": "≤150 chars, verbatim",
    "source_urls": ["urls"],
    "confidence": 0-100,
...
"""
    text = _gemini_api_call(prompt, max_output_tokens=8192)
    if not text:
        return grid, weighted_taxonomy
    try:
...
TODAY is {TODAY}. Lookback: {LOOKBACK_START} to {TODAY}.
Search Google for these MISS catalyst categories. Return any that actually occurred as a JSON array.

MISS catalysts: {json.dumps(miss_catalysts, indent=2)}
...
"""
    text = _gemini_api_call(prompt, max_output_tokens=4096)
    if not text:
        return []
    try:
        if text.startswith("```"): text = text.split("\n", 1)[1].rsplit("```", 1)[0].strip()
...
TASKS:
A. Merge Finviz events + raw events. Give Finviz events a confidence boost (+10 over raw). Deduplicate by description.
B. Classify every event into one or more catalyst categories from the taxonomy. Use EXACT taxonomy label.
C. Build the FULL catalyst grid (66 items). Status: HIT / MISS / N/A. For each HIT you MUST include: event_date, evidence_excerpt, source_urls, confidence, type, category, base_weight, adjusted_weight, sensitivity_rationale.
...
# ── Field normalizer ────────────────────────────────────
def normalize_catalyst_grid(grid, weighted_taxonomy, raw_events, finviz_events):
    """Ensure every grid item has all required fields, pulling from taxonomy and events."""
    # Build a map from taxonomy to weighted details
    for entry in grid:
        # standard taxonomy aliases
        ...
        if "adjusted_weight" not in entry or entry.get("adjusted_weight") is None:
            # from weighted taxonomy
            wt = weighted_taxonomy.get(label, {})
            entry["adjusted_weight"] = wt.get("adjusted_weight", entry["base_weight"])
        # event_date – try to find in linked raw events
        if ("event_date" not in entry or not entry.get("event_date")) and entry.get("status") == "HIT":
            # look for matching description in raw events or finviz events
            desc = entry.get("description") or entry.get("evidence_excerpt", "")
            if desc:
                for ev in raw_events + finviz_events:
                    if ev.get("description", "")[:80] == desc[:80]:
                        entry["event_date"] = ev.get("event_date", "?")
                        entry["source_urls"] = ev.get("source_urls", [])
                        entry["evidence_excerpt"] = ev.get("evidence_excerpt", "")
                        break
            if "event_date" not in entry:
                entry["event_date"] = "?"
        if "source_urls" not in entry:
            entry["source_urls"] = []
        if "evidence_excerpt" not in entry and entry.get("status") == "HIT":
            entry["evidence_excerpt"] = entry.get("description", "")
        if "confidence" not in entry or entry.get("confidence") is None:
            entry["confidence"] = 75
...
async def analyze_stock_async(ticker, snapshot, searxng_url):
    ...
    # Normalize fields
    grid = normalize_catalyst_grid(final_result.get("catalyst_grid", []), weighted_taxonomy, raw_events, finviz_events)
    print("  🐞 First grid item keys:", list(grid[0].keys()) if grid else "none")

    # Gemini factcheck (with delay to avoid rate limit)
    if GEMINI_API_KEY:
        time.sleep(5)   # short pause between LLM and Gemini
    grid, weighted_taxonomy = gemini_factcheck(full_name, ticker, grid, snapshot, weighted_taxonomy, TAXONOMY_LIST)

    # Gemini miss filler (with additional delay)
    misses = [c for c in grid if c.get("status") == "MISS"]
    new_hits = []
    if GEMINI_API_KEY and misses:
