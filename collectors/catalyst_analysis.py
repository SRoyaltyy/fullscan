...
Robust field population via event IDs – eliminates `?` and `None` in the final grid.
"""
...
def gemini_unified_check(full_name, ticker, grid, snapshot, weighted_taxonomy, taxonomy_list):
    ...
    # Build a compact representation of the grid
    compact_grid = []
    for c in grid:
        compact_grid.append({
...
3. For N/A items: sanity check.
4. Check sensitivity multipliers: are they appropriate for this company? If not, provide overrides.
...
{{
  "corrected_grid": [
    {{
      ...
      "correction_type": "confirmed|reclassified|new_find|source_disputed|date_corrected|multiplier_override",
  ...
  "new_hits_from_miss": [   ← events found for previously MISS catalysts
    {{
      "taxonomy": "label",
...
"""
    ...
    for attempt in range(4):
        ...
        print("  ❌ All Gemini attempts failed; skipping Gemini layer.")
        return grid, weighted_taxonomy, []
...
# Predefined positive catalyst set for type inference
...
B. Build the FULL catalyst grid (66 items). For each catalyst, set status: HIT / MISS / N/A.
   - For each HIT, you MUST copy the following fields EXACTLY from the event: event_date, evidence_excerpt, source_urls, confidence.
   - Also include the EVENT_ID numbers that contributed to this HIT (list of integers).
   - Use the adjusted_weight from the weighted taxonomy for that catalyst.
   - For MISS or N/A, no event data is needed.
...
{{
  ...
  "catalyst_grid": [
    {{
      ...
      "adjusted_weight": 10,
      "event_ids": [0, 3],
      "event_date": "2025-10-14",          ← from event #0
      "evidence_excerpt": "...",           ← from event #0
      "source_urls": ["https://..."],      ← from event #0
      "confidence": 90
    }},
    ... every catalyst
  ],
...
async def analyze_stock_async(ticker, snapshot, searxng_url):
    ...
    merged_events = []
    idx = 0
    # Finviz first, then raw events (so Finviz events get priority)
    ...
    for entry in grid:
        ...
        # Populate fields from event IDs
    ...
    # Gemini unified check
    if GEMINI_API_KEY:
        time.sleep(3)  # allow RPM breathing room after LLM
