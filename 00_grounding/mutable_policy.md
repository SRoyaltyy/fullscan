...
corrected_behavior: "When the sector's 1d relative tape is flat (|1d rel| < ~0.15%) and credit is tight and futures are mixed, do NOT score S1 as an independent negative on the strength of the macro narrative alone — the sector-specific transmission channel must be confirmed by 
...
corrected_behavior: "Keep the current treatment. When (a) the sector is a rate-sensitive bond-proxy, (b) the live long-end curve is verified rising at the open (not a stale prior-close table), (c) a live escalating oil/inflation overlay is present, an
...
corrected_behavior: "When a live macro overlay is present with (a) oil supply shock, (b) long-end yield backup, (c) 5-day yield–equity correlation ≤ −0.9, and (d) VIX term structure in backwardation, the sector's trailing
...
corrected_behavior: "(1) When 1d rel has stabilized for ≥2 consecutive sessions after a multi-day lag, decay S0/S1/S2 toward zero — cap the combined S0+S1+S2 at ≈ −1.5, not −3.0. (2) Broaden the double-count check: if S0, S1, and S2 all tr
...
corrected_behavior: "When the sector is deeply oversold (RSI < 30) AND the 1m relative lag is extreme (≤ −5%), treat the prior-day 1d relative tape as a decaying signal, not a level signal: keep the directional call (down/mild) — do NOT flatten — but recognize that the S4 confirm
...
corrected_behavior: "Cap any sector-specific-drag override of a regime signal at ONE session. To keep the drag at full weight on a subsequent day, require a FRESH negative print (new guidance cut, new data, new downgrade) that morning; absent one, the drag reverts to a car
...
falsifier: "If a same-morning Governor/Chair interview is on the da
...
### fresh-overnight-kinetic-oil-supply-increment-houthi-attack-o.md
---
...
### on-1w-books-micro-small-names-with-maxed-peer-rs-and-positiv.md
---
...
evidence_cited: "CYPH 2026-08-21 gated_out (join 0.102, AB 0.
...
corrected_behavior: "When a >+1% relative reversal occurs but (a) the sector's 1w
...
corrected_behavior: "When a sector has run 1w r
...
corrected_behavior: "When the 8/18 meta
...
corrected_behavior: "No correction needed — this is a template-following success. The S0 = −2 dominant-driver treatment, S1 = −1 transmission-channel separation, and mild magnitude cap all proved correct. The only marginal note: XLY at −1.34% was at the upper edge of mild (approachin
...
corrected_behavior: "When the narrative essay and pipeline disagree, the model must reconcile toward the narrative when the narrative's directional drivers are (a) a live, escalating macro shock (oil >$100, confirmed by futures down) and (b) confirming negative sector tape (1d rel negati
...
corrected_behavior: "When a live oil supply shock (Brent >$100) with long-end yields in stress zone is present, score S0=−2 (oil→inflation→long-end yield channel is a direct negative for rate-sensitive financi

### when-a-live-geopolitical-oil-supply-shock-wti-spiking-3-towa.md
---
...
corrected_behavior: "When a scheduled high-impact macro release is pending AND the pre-existing regime is already hawkish (rising real yields, negative 10Y-SPX corr), the model must pre-score asymmetric downside for long-duration
...
### when-a-sector-etf-has-a-composition-where-the-dominant-sleev.md
---
...
corrected_behavior: "When a fresh overnight catalyst is present but the broad tape is deeply risk-off AND the sector's magnitude hit-rate is ≤0.3, the mod
...
corrected_behavior: "Before applying the cushion-as-flat-override lesson, check whether the catalyst that created the cushion is escalating, static, or fading. If the shock is static (oil holding at the same level, no new kinetic escalation) or fading, the cushio
...
corrected_behavior: "When S0 is negative due to a broad macro drag (oil shock + hawkish Fed 
...
corrected_behavior: "Continue this pattern: when a sector-specific negative cluster has demonstrated dominance over the FTS bid on consecutive sessions, score S0=0 or negative even under the strongest FTS trigger (oil >$100); treat the oil spike as an input-cost negative for staples (S1), not a defensive bid (S0
...
### when-earnings-land-inside-the-1w-horizon-deeply-negative-s-j.md
---
...
evidence_cited: "CRM 2026-08-21 outweighed i

### when-nq-futures-are-holding-up-significantly-better-than-es.md
---
...
corrected_behavior: "When NQ futures are outperforming ES futur
...
evidence_cited: "CHPT 2026-08-27 gated_out (join -0.315, AB 0.635, news 0.0,
...
corrected_behavior: "When the narrative essay's final call diverges from the pipeline-computed decision, the model must explicitly reconcile: (a) state the pipeline output, (b) state the narrative output, (c) identify which components drive the disagreement, 
...
### scope `sector_healthcare` — wins=9 losses=6
