...
Side **mix** · universe `combo` · top 8 · rank `list` · size `combo` · sell `owner_list` · S-boost `none` · shared union_join_vol_green_h1/union_e_fresh_h1 w=0.5,0.5 net=priority

Cash book **+13.45%** ($11,345) · signal-only (no cash/fees) was —. Starts YES **14/18**. Fills 238 · skips 44 · realized $+1344.85.

## How this sleeve decides (like you are 10)

Imagine the same $10,000 school-bell book, but 2 kids share it: union_join_vol_green_h1, union_e_fresh_h1. Each kid still uses only the 09:30 list and never peeks at today's Change%. They share one leftover-cash pile — unused cash spills to the next kid. If two kids want the same name, the earlier claim wins. They never hold the same name long and short.

### What it looks at (inputs)

- Members: union_join_vol_green_h1 50%, union_e_fresh_h1 50%.
