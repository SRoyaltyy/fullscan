...
Open cross-check uses theme-radar `{D}.raw.csv` Finviz Open when that fetch's scrape_ts is after D 09:30 ET and before the next session's 09:30 ET, and the sha256 matches HASHES.json. From 2026-09-25 the slim `{D}.csv` Open column (appended last) is the next file under the same guard. A missing or late scrape uses Stooq for that day. `current.csv` is not a source. Webull paper fills stay a third check. On this window, 2026-09-24 uses `2026-09-24.raw.csv` (scrape_ts `2026-09-24T23:22:06+00:00`). Earlier sessions have no scrape_ts, so they use Stooq. The log is `data/factor_mine/open_source_log.csv`. Per-recipe session returns for the shuffle test are `data/factor_mine/daily_returns.csv` (recipe, start_date, D, ret as percent versus the prior close equity).
