...
- status: protocol locked. Tonight's initial run is Group 3 only: the original 110 Factor Mine recipes on proven inputs. Groups 1 and 2 are DEFERRED. The extended Layer B cross stays a later add-on. The Excel ML spec is reserved as one luck-test try and is not run tonight. Results are APPEND-ONLY FOREVER. This commit has no returns, no p-values, and no luck-test output.
- written: 2026-09-26
...
Tonight's initial run is Group 3 only. N = 110. The luck-test denominator is 110 + 37 + 8,264 + 868 + 1 = 9,280. The 1 is the Excel ML spec: one reserved try, not unpacked, and not run tonight. Groups 1 and 2 are DEFERRED. Scoring waits for the reply `go`. The initial Group 3 result is due by 23:00 HKT on Saturday 2026-09-26, with `research/audit/FULLSCAN_FILE_PROOF.csv` pinned at commit `1bf94d7dc3ce00c29667d0fae6fdb6bb8effb73c`.

The walk is the original Factor Mine engine from [PR #126](https://github.com/SRoyaltyy/fullscan/pull/126), commit `8e8c36a7117e60040d04cfa598e503b82fd7c6ea`: one session at a time, from inputs knowable at 09:30, with the previous day's locked positions carried forward. A past buy or sell is not re-picked. No locked file is rewritten. The scored run writes each session once under `research/lever_search/returns/factor_mine_seq/`. That record is APPEND-ONLY FOREVER. The 110 recipes in that engine's `build_recipes()` are Group 3. They are tonight's run. There is no re-picking.

## Append-only forever

Results are APPEND-ONLY FOREVER. This is a hard requirement. This commit writes none of the return files.

Each session's picks, fills, and returns for each recipe are written once to a per-day file under `research/lever_search/returns/factor_mine_seq/`. The file name is `{session}.json`. Its sha256 is recorded on one line of the append-only manifest `research/lever_search/returns/factor_mine_seq/manifest.jsonl`. A line is one JSON object. Lines stay in the order they were written. A later line is a later session.

A later rebuild may only add sessions after the last recorded one. It must reproduce nothing and change nothing earlier. The CI test `.github/workflows/lever_search_append_only.yml` fails the PR when any existing per-day file, manifest line, or earlier return changes, or when a manifest line is removed or reordered. The check is `python3 -m src.lever_search_append --check-against` the PR base sha.

Each manifest line pins the input snapshots used that day by git blob sha. A later rebuild cannot silently read a newer version of an input. The blob sha on an existing line is part of that line, and the line cannot change.

If inputs, code, or recipes ever need to change, that is a NEW study under a new name. Its tries are added to the tally. `factor_mine_seq` stays frozen as it is.

The per-day file holds every recipe for that session: picks, fills, `ret_futubull`, and `ret_flat_15bp`, plus the share of fills with fill price strictly under $3. A sit-out session is still a file. It carries the locked book and contains no fill built from a missing input.
...
Group 1 is every recipe whose signal does not include an Excel card and does not read a fullscan input. Its start day falls in 2026-08-07 through 2026-08-13. Those signals read prices and the 15 plain Finviz fields behind the 16 `fv_*` atoms, so every one of them would start 2026-08-07. Group 1 is DEFERRED: specified and fingerprinted, not run tonight. Group 2 is every recipe whose signal includes an Excel card and does not read a fullscan input. It would start 2026-08-31 and run only on Excel's seven server-proven sessions: 2026-08-31, 2026-09-02, 2026-09-03, 2026-09-04, 2026-09-08, 2026-09-10, and 2026-09-11. A recipe that reads both an Excel card and a Finviz atom needs both present that morning. All seven Excel sessions have a Finviz snapshot. Group 2 is DEFERRED: specified and fingerprinted, not run tonight. Group 3 is the 110 original Factor Mine recipes, and it is the only group in tonight's run. The recipes are sequential, with no re-picking. The candidate list, the attached inputs, and each recipe's search days and check days are the Group 3 rule below. Excel columns are used only on Excel's proven days. Tonight's luck test uses one denominator, 9,280. If Group 1 or Group 2 is ever run, its tries are added to the tally at that time, and every Group 3 result is re-tested against the enlarged tally. A Group 3 recipe that fails that re-test is not something good.

Fullscan inputs are pinned from `research/audit/FULLSCAN_FILE_PROOF.csv` at commit `1bf94d7dc3ce00c29667d0fae6fdb6bb8effb73c` on PR #354, branch `cursor/input-provenance-336-fcf3`. The git blob sha is `832eaa4fdc19d368d87a4dad5c3edd3c200b0a54`. The content sha256 is `1bfafd9d4f9a512b9bc3ac33d6d087349028f57ec4d0fedffe76ee0998e48f75`. This study freezes that commit and that blob. A later commit on #354 is not this pin. This commit does not vendor the CSV.

A day is usable for an input when every row of that input on that date has `proven=yes` and the path does not end with `/`. `PROVEN_BUT_CHANGED` counts, because only the pre-open blob is read. Headline inputs also need `stale_content=no` on every row. Headline inputs are actions, baseline, catalyst, digest, events, judge, map_heat, market_digest, parsed, and research. Any other row, a blank, or a missing row sits that input out. No fill is written from it.

On the audit window of 31 sessions from 2026-08-13 through 2026-09-25, the usable-day counts are stock_book 19, S 27, predict 27, hard_red 27, sector_predict 21, ab_checklist 18, ab_enriched 18, actions 12, judge 6, map_heat 4, and catalyst 2. AB checklist and AB enriched are not the same 18 days. A recipe that reads AB needs both files on the same day: 16 audit days, and 10 of those sit inside the run window.

On a day where stock_book is usable, the Group 3 candidate list is that day's proven stock_book blob. S, predict, and hard_red are attached on their 27 usable days. sector_predict is attached on its 21 usable days. AB checklist and AB enriched are attached only on a day both are usable. Actions, judge, map_heat, and catalyst are attached only on their usable days. The original 110 recipes have no S gate and no hard-red gate, so those 27 days do not by themselves add a sit-out. No original-110 gate key maps to sector_predict, so those 21 days do not by themselves add a sit-out. A recipe that needs a missing attached input sits that day out. Days without a usable stock_book use the price-only candidate list: prior-day gainers, prior-day movers, the hot list, and overnight, computed fresh from bars dated strictly before the session. `flatten` is not a price-only source. It runs only on a stock_book-usable day and reads the flatten names from that blob. `probable`, `yday_gainer`, and `ohlc_hot` are price-only and do not need stock_book unless a gate requires another file. `probable` is the original engine's continuation list, computed from those bars. Today's `data/factor_mine/panel.json` rows are never the candidate list. Theme Radar mornings are not the Group 3 candidate list.

Do not recompute AB Part A (A1-A15). The audit shows today's Part A does not reproduce the server-proven pre-open A1-A15 files, 0/3 days: 2026-09-21, 2026-09-22, and 2026-09-24. Group 3 reads AB only from the proven per-day ab_checklist and ab_enriched files, and only on a day both are usable. Section 7's price-only A atoms stay the deferred Layer A spec. Group 3 does not use that recomputed path.

Search days are the run-window sessions on which every fullscan role the recipe requires is usable. Check days are those search days that also sit on the 16-day check calendar. A sit-out carries the locked book, has no new fill, and is not a search day and not a check day. If a recipe has zero search days, it has zero check days. Within each input group every recipe has the same counts, so that group's min, median, and max are that one count.

Group 3 overall, 110 recipes: search days min 0, median 10, max 25. Check days min 0, median 7, max 16.

| group | recipes | search days | check days |
| --- | ---: | ---: | ---: |
| stock_book only | 48 | 10 | 7 |
| actions and stock_book | 22 | 4 | 1 |
| no fullscan role (price-only, or union with the price-only fallback) | 16 | 25 | 16 |
| export and stock_book | 6 | 8 | 5 |
| join and stock_book | 4 | 0 | 0 |
| ab, actions, and stock_book | 2 | 1 | 1 |
| ab and stock_book | 2 | 5 | 5 |
| actions only | 2 | 11 | 6 |
| actions, export, and stock_book | 2 | 3 | 0 |
| actions, join, and stock_book | 2 | 0 | 0 |
| catalyst and stock_book | 2 | 0 | 0 |
| export only | 2 | 14 | 11 |

The stock_book-only search days inside the run window are 2026-08-13, 2026-08-14, 2026-08-17, 2026-08-27, 2026-08-31, 2026-09-04, 2026-09-08, 2026-09-09, 2026-09-10, and 2026-09-11. Seven of those sit on the check calendar: 2026-08-27, 2026-08-31, 2026-09-04, 2026-09-08, 2026-09-09, 2026-09-10, and 2026-09-11. Catalyst usable days are 2026-08-28 and 2026-09-25. 2026-08-28 is not a stock_book day, and 2026-09-25 is outside the run window, so the catalyst and stock_book intersection is empty.

The rolling walk-forward check is section 6. The first check day is 2026-08-20. Training lookback is expanding: the choice set for a recipe is every run-window session strictly before the check day and on or after that recipe's start day. For a recipe that starts 2026-08-07, the first check day's training set is nine sessions: 2026-08-07, 2026-08-10, 2026-08-11, 2026-08-12, 2026-08-13, 2026-08-14, 2026-08-17, 2026-08-18, and 2026-08-19. For a recipe whose columns start 2026-08-13, that training set is the five sessions 2026-08-13, 2026-08-14, 2026-08-17, 2026-08-18, and 2026-08-19. The check calendar runs through 2026-09-11 and has 16 sessions. For Groups 1 and 2, a recipe's own check days are the calendar sessions on or after its start day. A sit-out session in that span still counts for those deferred groups, and it has no new fill. Group 3 check days are the search days in the Group 3 rule above. A Group 3 sit-out is not a check day. Group 1 recipes that start on or before 2026-08-20 have 16 check days. Group 2 starts 2026-08-31, so it has 9 check days: 2026-08-31, 2026-09-01, 2026-09-02, 2026-09-03, 2026-09-04, 2026-09-08, 2026-09-09, 2026-09-10, and 2026-09-11. 2026-09-07 is Labor Day and is not a session.

### Inputs loaded

Price levers are the Layer A price atoms, candle atoms, and price-only AB atoms in section 7, their pairs with each other, and the six price deltas. On 2026-08-07 through 2026-09-08 the candidate universe is the window rule above. The price-only leg of that universe is section 7.1. The daily series for a price-only signal runs 2024-04-02 through 2026-09-11. These price inputs are **PROVISIONAL**. `research/lever_search/INPUT_ASOF_MANIFEST.json`, sha256 `920f5a1c402a7dcc102e6ed8e47cd7be9f33243243cd73aff65e6f70655e3acd`, records `rebuild_match` as null on the price store, the listed roster, `src/ab_checklist.py`, and `weather_rules.json`. A recipe is not something good while an input it relies on is provisional. If the audit does not set `rebuild_match` to `exact` for that input, every recipe that relies on it is struck. Fills read `data/prices/ohlc.parquet`, so a failed byte check of that snapshot strikes the whole initial run. The price store's Actions run start is 2026-09-25T20:50:47Z. That is not a before-09:30 copy of any search session, and the git committer date is not proof. Group 3 price features are computed fresh with the frozen engine from commit `8e8c36a7117e60040d04cfa598e503b82fd7c6ea` and do not need to match old per-day files. The features are hot score, returns, rvol, breakout, candles, RSI, MACD, peer RS, and the price-only sources (prior-day gainers, prior-day movers, the hot list, and overnight). The bar snapshot is `data/prices/ohlc.parquet` at git commit `ff996f535e1343dd739cc801780ae224018bd96c`, blob sha `3456f7f489a6fa7033e8ae5cc942d8279f0113e3`, content sha256 `559c8cf099808930bef2b4de4280b4e902883c9a1de85c8a417074f11aaefa55`. Yahoo download uses `auto_adjust=false`: the bars are split-adjusted, and dividends are not applied. A feature bar must be dated strictly before the session. The session open is read only when the frozen recipe trades at the 09:30 open, so the gap versus the prior close is known at 09:30. All 110 frozen recipes trade at the 09:30 open, so they may read that session's open for the fill and the gap. They may not read that session's high, low, or close, and they may not read any bar dated the session or later as a feature bar. `.github/workflows/lever_search_append_only.yml` runs `python3 -m src.test_lever_search_bars`, which fails if a same-day-or-later bar is read. A mismatch with an old per-day price file does not strike a recipe. A failed byte check of this bar snapshot still does, because every fill and every feature reads it.

Theme Radar and Finviz levers are **available-proven** for every export morning in `SRoyaltyy/theme-radar` commit `19973230e4d80c74565e1246ada911503a808fb7`, file `research/lever_panel/server_time_proof.csv`, sha256 `5bde5e9164f499899b4ed70fcdbfde3f72b3e8b5692da645f1e9682e3c6a913e`. That file has 234 rows, all `PROVEN`, on 35 trade dates. Twenty-four of those dates sit inside 2026-08-07 through 2026-09-11. The four mornings before 2026-08-13 are 2026-08-07, 2026-08-10, 2026-08-11, and 2026-08-12. The other twenty are 2026-08-13, 2026-08-14, 2026-08-17, 2026-08-18, 2026-08-19, 2026-08-20, 2026-08-21, 2026-08-24, 2026-08-25, 2026-08-26, 2026-08-27, 2026-08-31, 2026-09-01, 2026-09-02, 2026-09-03, 2026-09-04, 2026-09-08, 2026-09-09, 2026-09-10, and 2026-09-11. Those proven Theme Radar mornings are the Finviz read for the deferred Group 1 `fv_*` atoms. They are not tonight's Group 3 candidate list. The 16 `fv_*` atoms are Group 1. They are DEFERRED and are not scored tonight. If Group 1 is ever run, that read loads Finviz on those 24 only, and only the 15 Finviz columns behind the 16 `fv_*` atoms. Mornings after 2026-09-11 stay out of the search load. A sample of eight cited runs was read with `gh api repos/SRoyaltyy/theme-radar/actions/runs/{id}`: 31136381205, 31642191661, 31645482181, 33026531289, 33814488206, 34539436709, 34541105465, and 36187437404. On each one, `run_started_at` matched the file's `created_at` and was before 13:30 UTC on that trade date. Rubric scores, composite scores, catalyst flags, upside, `trf_d_*`, and `trf_dir_*` are available-proven on those mornings and are not loaded on this run.

Excel signal columns (`ticker`, `strategy`, `signal_date`) are allowed only on server-proven days in `research/lever_search/excel_preopen_proof.csv`, sha256 `04e6b996c450427ea408646b305be91c33f0778c1f6ee167de9be1a08d129ded`. Inside the run window those days are 2026-08-31, 2026-09-02, 2026-09-03, 2026-09-04, 2026-09-08, 2026-09-10, and 2026-09-11. Every other session from 2026-08-07 through 2026-09-11 is sat out for a recipe that reads an Excel card: 2026-08-07, 2026-08-10, 2026-08-11, 2026-08-12, 2026-08-13, 2026-08-14, 2026-08-17, 2026-08-18, 2026-08-19, 2026-08-20, 2026-08-21, 2026-08-24, 2026-08-25, 2026-08-26, 2026-08-27, 2026-08-28, 2026-09-01, and 2026-09-09. Post-open rows are dropped even on a proven day: CMII on 2026-09-02, AUBN on 2026-09-04, and SVCC on 2026-09-11. Price and return columns stay outcomes. Pre-09-14 Excel signals were computed from mid-session prices. That is clock-legal: each pre-open copy was frozen before the next open. The initial Group 3 result is due by 23:00 HKT on Saturday 2026-09-26, with `research/audit/FULLSCAN_FILE_PROOF.csv` pinned at commit `1bf94d7dc3ce00c29667d0fae6fdb6bb8effb73c`.
...
- Today's `data/factor_mine/panel.json` rows as the candidate list on any session.
...
The initial run is what scores first. It holds Group 3 only: the original Factor Mine recipe space, the 110 recipes, walked on the pinned `research/audit/FULLSCAN_FILE_PROOF.csv` usable days, with the stock_book candidate list on stock_book-usable days and the price-only sources otherwise, and with Excel columns only on Excel's proven days. Groups 1 and 2 are DEFERRED. The extended cross N_B = 1,984,711,680 stays a later add-on. The Excel ML spec is the reserved 1 inside the luck-test denominator and is not run tonight. A combination already inside N is not counted again.
...
`data/factor_mine/panel.json`, sha256 `f9a4efc13a8aa13b92ac5847f0ef4f3dc4bddf0b16652684b183a9fe3170a506`, is not a candidate source. The initial run does not load its rows.

The file's own session dates are these 21, and no others:
...
Those 110 are the engine's enumerated list. They are Group 3, and each one is one try in N. The extended cross below is not the original recipe space and is not in N.
...
`union` is that morning's rebuilt candidate list: names from per-day files marked `PROVEN`, plus proven Theme Radar names and the price-only list. `flatten`, `probable`, `yday_gainer`, and `ohlc_hot` keep a name only when a `PROVEN` per-day file carries that list. `theme_radar` is the frozen export for that `trade_date`. A camera gate whose file is not `PROVEN` that morning matches nobody. `panel.json` rows are not a source. That sentence is the deferred grid. Tonight's Group 3 candidate list is the stock_book rule in the Initial run section.
...
| top-N | `1`, `4`, `8`, `12` |
| universe | `union` (that morning's rebuilt candidate list), `flatten`, `probable`, `yday_gainer`, `ohlc_hot` (a `PROVEN` per-day file must carry the name), `theme_radar` (that `trade_date` on the frozen export) |
...
Price-only AB atoms. The flag is the committed `_pass_a` value, and the atom is true when that flag equals 1. Source is `src/ab_checklist.py` at git `01d6380c8ed6dfff34afc78feed675386b26bf68` plus the A15 patch in the current `src/ab_checklist.py`. Part B is not read. That table is the deferred Layer A spec. Group 3 does not recompute A1-A15. The audit shows Part A does not reproduce, 0/3. Group 3 uses AB only from proven per-day files.
...
This commit writes none of the return files. Results are APPEND-ONLY FOREVER, as specified above. The record of a session is `{session}.json` under `research/lever_search/returns/factor_mine_seq/`, one line in `manifest.jsonl`, and the sha256 of that file. Both fee columns are required on that file: `ret_futubull` and `ret_flat_15bp`. The scored run records every recipe, not only the top recipes. No locked ledger, scoreboard, panel, or prior return file is rewritten. An existing per-day file is not replaced, and a new sibling is not written in its place.

The series for a recipe is the concatenation of those per-day files in manifest order. Price-only signals include the 614 sessions from 2024-04-02 through 2026-09-11. Signals that include an `fv_*` atom include the 25 sessions from 2026-08-07 through 2026-09-11. Signals that include an Excel card include the 9 sessions from 2026-08-31 through 2026-09-11. A session the recipe sits out is still a per-day file. The file carries the locked book and contains no fill built from a missing input. The report computed from those files records `n_check_days` and the mean after-fee return per check day, under Futubull fees and under the flat 15bp. It also records the share of trades in names under $3: the count of fills with fill price strictly under $3, divided by the number of fills. No price filter is added. That report is not a per-day file and it is not a reason to change one. The designed-after sessions, 2026-09-14 through 2026-09-25, are later per-day files appended after the last recorded session, and only after the walk-forward choices are frozen. They are not written into an earlier file.

Both fee series are inputs to that recipe's luck test. The prior 8,264 and 868 keep the fingerprinted series already named in section 8. They are not rewritten.

The report also lists the top recipes, the after-fee return with the single best stock removed, and the after-fee result from 2026-09-14 on. Those lists do not replace the per-day files.

The yearly breakdown is computed from the frozen per-day files. It is not stored inside `factor_mine_seq`, and it is not a second copy of a day's returns.
...
The scored run refuses to start when the header fingerprint disagrees with the covered bytes, when a pinned file's sha256 disagrees with the manifest, when a before-09:30 claim rests on a git author date or a git committer date, when a row dated 2026-09-14 or later is passed into the search or the walk-forward, when the search loader is given a session after 2026-09-11, when a loaded search row contains a `trade_date` after 2026-09-11, when a Theme Radar date outside the 24 proven mornings is requested, when a dropped Excel session or a dropped Excel pair is requested, when an Excel read asks for a price or return column, when an initial Finviz read asks for a column outside the 15 Finviz fields, when a price-only signal opens `panel.json`, a Theme Radar file, or an LLM morning file, when today's `panel.json` rows are read as the candidate list on any session, when a fullscan input is used on a session the pinned `FULLSCAN_FILE_PROOF.csv` does not mark usable (`proven=yes` on every row, and `stale_content=no` for a headline input), or when a recipe opens a fill on a session where one of its own inputs is not server-proven, or when an existing per-day file, manifest line, or earlier return would change, or when a manifest line would be removed or reordered, or when a feature read includes a bar dated the session or later, or when Group 3 recomputes A1-A15.

## 11. Success criteria

Fixed before any score. A recipe is something good only when every line below holds.

(a) After Futubull fees, the cumulative return on the recipe's own check days is at least 20%. For Group 3 those days are the recipe's search days on the section 6 calendar, and a sit-out is not a check day. For Group 2, if it is ever run, those days are the nine calendar sessions from 2026-08-31 through 2026-09-11.
