...
- written: 2026-09-26
- fingerprint_sha256: 039009d8b209dbebd334c13e773beb27705854a356b03e3c3b05d298b30ce11c
...
On every session the candidate list is rebuilt from `PROVEN` per-day files and from proven Theme Radar and price sources only. Price sources stay provisional pending the audit. Today's `data/factor_mine/panel.json` rows are never the candidate list.
...
The scored run refuses to start when the header fingerprint disagrees with the covered bytes, when a pinned file's sha256 disagrees with the manifest, when a before-09:30 claim rests on a git author date or a git committer date, when a row dated 2026-09-14 or later is passed into the search or the walk-forward, when the search loader is given a session after 2026-09-11, when a loaded search row contains a `trade_date` after 2026-09-11, when a Theme Radar date outside the 24 proven mornings is requested, when a dropped Excel session or a dropped Excel pair is requested, when an Excel read asks for a price or return column, when an initial Finviz read asks for a column outside the 15 Finviz fields, when a price-only signal opens `panel.json`, a Theme Radar file, or an LLM morning file, when today's `panel.json` rows are read as the candidate list on any session, when a fullscan input is used on a session whose `FULLSCAN_FILE_PROOF.csv` status is not `PROVEN`, or when a recipe opens a fill on a session where one of its own inputs is not server-proven, or when an existing per-day file, manifest line, or earlier return would change, or when a manifest line would be removed or reordered.
