...
PREFIX = "fwd_ccap2_"
# Width-4 cap-none P2 return without the top stock, the corrected v4 keep-held reading.
CONTROL_EX_TOP1 = {
...
def is_rejected(frozen: bool, p2: dict) -> bool:
    """Joint under 0.5, best share at least 20% of positive P2 profit, or a negative ex-top-3."""
    if not frozen:
        return False
    start = p2.get("start_equity")
    end = p2.get("end_equity")
    profit = None if start is None or end is None else float(end) - float(start)
    share = p2.get("profit_share")
    joint_fail = (
        p2["n"] >= MIN_TRADES
    ...
    share_fail = (
        ...
        and float(share) >= SHARE_MAX
    )
    ex3 = p2.get("ex_top3")
    top3_fail = ex3 is not None and float(ex3) < 0.0
    return bool(joint_fail or share_fail or top3_fail)
...
def _write_hook(winners: list[dict]) -> None:
    ...
    if ours:
        if [canon(row) for row in ours] != [canon(row) for row in winners]:
            raise SystemExit("v4 hook already has a different concentration_cap_v2 carry")
        return
    if not winners:
        return
...
def _report(freeze: dict, tune_rows: list[dict], forward_rows: list[dict], iwm_tune, iwm_p2) -> str:
    ...
    lines = [
        ...
        f"Luck N is {LUCK_N}. That is {TRIES} tries in this study, plus {V1_TRIES} fingerprinted tries from concentration_cap_v1, plus 21,796.",
    ...
    if not carry:
        ...
        lines.append("| id | worst joint | tune return | tune best share | tune ex-top3 | P2 return | P2 best share | P2 ex-top3 | P2 trades | P2 win rate |")
        lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
        ...
        for ident in carry:
            ...
            lines.append(
                ...
                f"{_pct(tune['tune']['profit_share'])} | {_pct(tune['tune']['ex_top3'])} | "
                f"{_pct(fwd['p2']['compound'])} | {_pct(fwd['p2']['profit_share'])} | "
                f"{_pct(fwd['p2']['ex_top3'])} | {fwd['p2']['n']} | {_pct(fwd['p2']['win_rate'])} |"
            )
        lines.append("")
        lines.append("Each of these is appended to `research/forward_shadow_v1/hooks/v4_winners.json` as `fwd_ccap2_` plus the id. `first_session` is 2026-09-28.")
        lines.append("")
...
def main() -> None:
    ...
    for spec in candidates():
        ...
        for key in ("compound", "ex_top1", "ex_top3", "ex_top5", "n", "win_rate", "profit_share"):
