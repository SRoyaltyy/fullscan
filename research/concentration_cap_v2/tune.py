...
def is_passer(tune: dict, starts: list[dict]) -> bool:
    """Monday joints, positive profit, best share under 20%, and ex-top-3 above zero."""
    joints = [item["joint"] for item in starts if item["joint"] is not None]
    if len(joints) != len(STARTS):
        return False
    start = tune.get("start_equity")
    end = tune.get("end_equity")
    if start is None or end is None:
        return False
    profit = float(end) - float(start)
    share = tune.get("profit_share")
    ex3 = tune.get("ex_top3")
    return bool(
        profit > 0.0
...
def prereg_sha() -> str:
    proc = subprocess.run(
        ["git", "log", "-1", "--format=%H", "--", "research/concentration_cap_v2/PREREG.md"],
