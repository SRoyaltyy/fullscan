...
def score_tune(payload: dict, store, fees: dict) -> dict:
    ...
    for spec in candidates():
        ...
        for start in STARTS:
            ...
            starts.append({"joint": part["joint"], "n": part["n"], "start": start,
                           "up_share": part["up_share"], "win_rate": part["win_rate"]})
        joints = [item["joint"] for item in starts if item["joint"] is not None]
        passer = len(joints) == len(STARTS) and tune["ex_top3"] > 0.0
    ...
    order = [row["id"] for row in passers]
    return {
        "by_id": by_id,
        "iwm_tune": iwm,
        "passers": order,
...
def prereg_sha() -> str:
    import subprocess
    proc = subprocess.run(
        ["git", "log", "-1", "--format=%H", "--", "research/concentration_cap_v1/PREREG.md"],
