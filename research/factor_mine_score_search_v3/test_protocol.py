"""Locks for factor_mine_score_search_v3. No price file and no score."""
...
def test_fingerprint_and_inputs() -> None:
    ...
    assert file_sha256(ROOT / "research/factor_mine_score_search_v3/INPUTS.json") == INPUTS_SHA256
    assert INPUTS_SHA256 in text
    assert EXCEL_SHA256 in text
    assert "Luck N is 2754." in text
    assert LUCK_N == 2754
    assert (V1_CELLS, V2_CELLS, V3_CELLS) == (102, 1326, 1326)
    assert LUCK_N == V1_CELLS + V2_CELLS + V3_CELLS
    assert "Earnings surprise stays in the grid." in text
    assert "Excel colour is not a score formula." in text
    assert "Strict does not read panel.json." in text
    payload = load_inputs()
    assert list(payload["dates"]) == list(TUNE + FORWARD)
    assert payload["dates"]["2026-08-27"]["s_primary"] is None
    assert payload["dates"]["2026-08-26"]["earn_primary"]["on"] is False
    assert payload["dates"]["2026-08-24"]["primary_source"] == "price_only"
    assert hard_red(None) is True
    assert hard_red(0.0) is False
    got = []
    for session in TUNE:
        score = payload["dates"][session]["s_primary"]
    ...
    for session, block in payload["dates"].items():
        n_primary = len(block["stock_book_primary"]["names"]) if block["primary_source"] == "stock_book" else 0
        n_strict = len(block["stock_book_strict"]["names"]) if block["strict_source"] == "stock_book" else 0
        line = (
            f"| {session} | {cell(block['s_primary'])} | {cell(block['s_strict'])} | "
            f"{block['primary_source']} | {n_primary} | {block['strict_source']} | {n_strict} | "
            f"{'on' if block['earn_primary']['on'] else 'out'} | "
            f"{'on' if block['earn_strict']['on'] else 'out'} |"
        )
        assert line in text
    assert forward_status(29, 0.9, 0.9) == "unproven"
    assert forward_status(30, 0.4, 0.7) == "rejected"
    assert forward_status(30, 0.5, 0.7) == "proven"
...
def test_grid_and_required_cells() -> None:
    ...
    assert ("2026-08-21", 4) in cells
    assert ("2026-08-20", 2) not in cells
    assert ("2026-08-25", 4) not in cells
    assert all(top_n != 2 for _start, top_n in cells)
...
def test_prereg_precedes_the_freeze() -> None:
    log = _git(
        ...
        "research/factor_mine_score_search_v3/freeze/FREEZE.json",
    ).splitlines()
    if not log:
        return
    parent = _git("rev-parse", f"{log[-1]}^").strip()
    prereg_then = subprocess.check_output(
        ["git", "show", f"{parent}:research/factor_mine_score_search_v3/PREREG.md"], cwd=ROOT,
    )
    assert prereg_then == PREREG.read_bytes()
    missing = subprocess.run(
        ["git", "cat-file", "-e", f"{parent}:research/factor_mine_score_search_v3/freeze/FREEZE.json"],
        cwd=ROOT,
    )
...
def test_freeze_precedes_the_forward_file() -> None:
    log = _git(
        "log", "--diff-filter=A", "--format=%H", "--",
        "research/factor_mine_score_search_v3/returns/REPORT.md",
    ).splitlines()
    if not log:
        return
    parent = _git("rev-parse", f"{log[-1]}^").strip()
    ...
    missing = subprocess.run(
        ["git", "cat-file", "-e", f"{parent}:research/factor_mine_score_search_v3/returns/REPORT.md"],
        cwd=ROOT,
    )
    assert missing.returncode != 0
...
def main() -> None:
    ...
    print("factor_mine_score_search_v3 protocol ok")
