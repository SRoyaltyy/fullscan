# Factor Mine sequential clean-tape study — preregistration

- study: `factor_mine_seq_clean_tape`
- status: protocol locked. This commit has no returns, no p-values, and no luck-test output.
- written: 2026-09-26
- owner: Cyrus, 2026-09-26. Any input file labelled for trading day D may be used on D. It is assumed received before 09:30 ET. This study does not require a GitHub server time. The study label is `assumed pre-open, clean tape (split-artefact names absent or corrected)`.
- fingerprint_sha256: 5c29bd10af16edcfd94dda79254df06ac0a29b3e8077432f677ae4d53af5910b
...
- follows: [research/lever_search_labelled/PREREG.md](../lever_search_labelled/PREREG.md), except where this file says otherwise. Recipes are `build_recipes()` at `8e8c36a7117e60040d04cfa598e503b82fd7c6ea`.
- live paths: not used. No edits under `research/lever_search/` or `research/lever_search_labelled/`. No existing file is rewritten.

<!-- BEGIN COVERED -->

## What this study is

This is a new study. Its name is `factor_mine_seq_clean_tape`. It is not a rerun of `factor_mine_seq`, and not a rerun of `factor_mine_seq_labelled`. The 110 tries here are added to the luck-test tally. The earlier studies stay frozen.
...
Owner decision, Cyrus, 2026-09-26: an input file labelled for trading day D may be used on D, and it is assumed received before 09:30 ET. No Actions run start and no push-event time is required. `rebuild_match` does not strike a recipe. The whole study is labelled `assumed pre-open, clean tape (split-artefact names absent or corrected)`. That label is not a per-recipe strike.

This study re-runs the labelled study's recipes and inputs on one changed input: the price tape. Everything else follows `research/lever_search_labelled/PREREG.md`. Fees, fills, the cash book, the two fee columns, the share of fills under $3, and the four success lines are that protocol. The departures are this file.

## The clean tape

Bars are `research/breadth_rank_v1c/bars/ohlc.parquet` at git commit `ad10f862ad51df88f4dd03ff3dbcdf628f7e2685`, blob sha `e7bbac335cfa87243f9d0ddf8c331a0739dd0df8`, content sha256 `5c272584309e14496dc006a3a356c6960ed5b340f945af3fd6f299301b261ef2`. This is the PR #362 rebuild of the Yahoo bars: split artefacts repaired, and names whose prints could not be repaired are absent.

Differences from the labelled study's tape, and their consequences:

- REAX is present, with the 2026-08-19/2026-08-20 prints on the corrected post-split scale. The 10x artefact that inflated the labelled study's REAX-driven entries is not in this tape.
- ALP and the other unrepairable names are absent. A name absent from the tape has no feature and no fill. Its panel row stays in the candidate list; it is simply never filled and never marked.
- SPY is absent. `rs_week` is empty for every row. No recipe gates on `rs_week`, so no recipe is affected.
- The tape covers 2026-03-02 through 2026-09-25, so the 60-bar feature window is fully populated for every session.

Feature construction is unchanged: a feature bar is dated strictly before D, all 110 recipes trade at the 09:30 open, the session open is the fill and the gap, the session close is the mark only, and the session high and low are not read.
...
The labelled inputs are the labelled study's manifest, unchanged: the same earliest-commit pins for panel, stock_book, actions, ab_checklist, ab_enriched, export, catalyst, and join, for the same 31 sessions. Commit and blob sha for every `(input, D)` are in `research/lever_search_clean/INPUT_MANIFEST.json`, sha256 `0112bf246bbd947c04d508ca84709e799ba0f44f70686d7d4226c61d6e7f2d37`.
...
A path that is not in the tree has no manifest row. That input is absent on D. The per-session availability is the labelled study's, so the search-day and check-day counts are the labelled study's counts.
...
Bars are the clean tape pinned above. A feature bar is dated strictly before D. All 110 recipes trade at the 09:30 open, so the session open is the fill and the gap. The session close is the mark only. The session high and low are not read.
...
The luck-test denominator is 9,390 + 110 = 9,500. The 9,390 are the labelled study's denominator. The added 110 are this study's recipes. p is a one-sided t-test that the mean check-day Futubull return is above zero, multiplied by 9,500, and capped at 1. Fewer than two check days cannot reject the null.

## Append-only forever

This commit writes none of the return files.

Each session is written once to `research/lever_search_clean/returns/{session}.json`. Its sha256 is one line of `research/lever_search_clean/returns/manifest.jsonl`. A line is one JSON object with `session`, `file`, `sha256`, and `input_blobs`. Each blob entry has `path`, `blob_sha`, and `commit`. Lines stay in the order they were written. A later line is a later session.

A later rebuild may only add sessions after the last recorded one. `.github/workflows/lever_search_clean_append_only.yml` fails the pull request when an existing per-day file, manifest line, or earlier return changes, or when a manifest line is removed or reordered. The check is `python3 -m src.lever_search_clean_append --check-against` the pull-request base sha.

The report computed from those files is `research/lever_search_clean/returns/REPORT.md`. It is not a per-day file. It records, for every recipe, the side, the check-day count, the mean return per check day, both fee compounds and both fee means, the best stock removed and that ticker, the from-09-14 Futubull compound, the under-$3 share, and the luck p. It ranks all 110, and it lists the top 5 long recipes on their own.
